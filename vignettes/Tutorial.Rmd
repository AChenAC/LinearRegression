...
all.equal(as.numeric(unlist(ANOVA(mpg ~ cyl + wt, mtcars, type = "Sequential")["Df"])), 
          anova(lm(mpg ~ cyl + wt, mtcars))$"Df")
all.equal(as.numeric(unlist(ANOVA(mpg ~ cyl + wt, mtcars, type = "Sequential")["Sum Sq"])), 
          anova(lm(mpg ~ cyl + wt, mtcars))$"Sum Sq")
all.equal(as.numeric(unlist(ANOVA(mpg ~ cyl + wt, mtcars, type = "Sequential")["Mean Sq"])), 
          anova(lm(mpg ~ cyl + wt, mtcars))$"Mean Sq")
all.equal(as.numeric(unlist(ANOVA(mpg ~ cyl + wt, mtcars, type = "Sequential")["F value"])), 
          anova(lm(mpg ~ cyl + wt, mtcars))$"F value")
all.equal(as.numeric(unlist(ANOVA(mpg ~ cyl + wt, mtcars, type = "Sequential")["Pr(>F)"])), 
          anova(lm(mpg ~ cyl + wt, mtcars))$"Pr(>F)")
```

From above, we have shown that `ANOVA( )` and built-in function `anova( )` output the same results via column-wise comparisons through `all.equal( )`. Since the output of `ANOVA( )` is a data.frame, we could access each column or row by its corresponding name.
