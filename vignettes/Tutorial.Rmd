...
plot(mark(MyNumeric, RNumeric))
```

From above, we have shown that `ANOVA( )` and built-in function `anova( )` output the same numeric results via `all.equal( )`. `ANOVA( )` slightly outperforms `anova( )` in terms of speed. 
...
Again, we have shown that `ANOVA( )` and original `r` function `Anova( )` in cars package output the same numeric results via `all.equal( )`. In this case, both function performs equally good. 
