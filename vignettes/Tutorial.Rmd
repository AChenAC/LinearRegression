...
The LinearRegression package aims to provide almost every piece of information needed for linear regression, except diagnostic. The `LR( )` function mimics the output from `lm( )`, `summary(lm( ))`, `confint( )`, `hatvalues(lm( ))`, `rstandard( )`, and `rstudent( )`. 

```{r setup}
library(LinearRegression)
```

Let's first demonstrate a few example of the use of `LR( )`.
...
plot(bench::mark(as.numeric(model$coeff_summary), as.numeric(summary(model.r)$coefficients)))
```
...
```{r fig.align ='center', fig.width= 7, fig.height = 5, message=FALSE}
MyPredict = LR(cyl ~ mpg + wt, mtcars, to.predict = matrix(c(mean(mtcars$mpg), 18, mean(mtcars$wt), 4), 2, 2))$predicted
print(MyPredict)
RPredict = predict(lm(cyl~mpg+wt,mtcars), newdata=data.frame(mpg=c(mean(mtcars$mpg),18), wt = c(mean(mtcars$wt),4)))
print(RPredict)
all.equal(as.numeric(MyPredict), as.numeric(RPredict))
plot(bench::mark(as.numeric(MyPredict), as.numeric(RPredict)))
```
...
plot(bench::mark(as.numeric(model2$CI), as.numeric(confint(model.r2))))
```
